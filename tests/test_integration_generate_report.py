...
def test_generate_report_end_to_end_crea_excel_en_processed_data(
    ...
    ok_pattern = r"\[OK\] Informe guardado en: processed_data[\\/]informe_\d{8}_\d{6}\.xlsx"
    assert re.search(ok_pattern, out)

    generated = list((tmp_path / "processed_data").glob("informe_*.xlsx"))
